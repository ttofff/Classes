...
void BossScene::update(float dt)
{
	...
	if (lasttime - time > 1.0)
	{
		lasttime = time;
		char text[10];
		sprintf(text, "%d", (int)lasttime);
		totalWaveText->setString(text);
	}
	if (time<0)
	{
		CarrotDead();
	...
	if (monsterNum > 0)
	{
		if (waittime <= 0.f)
		{
			...
			newmonster->hp = newmonster->hp * 100;
...
bool BossScene::init()
{
	...
	// 生成关卡图
	__String *str;
	str = __String::createWithFormat("Map\\BossKind\\Environment\\%d\\Level.tmx",index);
	...
	//总波数
	totalWaveText = TextAtlas::create("60", "Themes\\Items\\numwhite-hd.png", 20, 40, ".");
	totalWaveText->setPosition(Vec2(480, 45));
	menuBG->addChild(totalWaveText, 2);

	// 全局速度变量
	CCScheduler* pSchedulers = CCDirector::sharedDirector()->getScheduler();
	pSchedulers->setTimeScale(1.f);
	...
	// 2倍速按钮
	Button* btn_speed_t = Button::create("Themes\\Items\\touming-hd\\speed21.png", "Themes\\Items\\touming-hd\\speed22.png");
	btn_speed_t->setPosition(Vec2(725, 605));
	this->addChild(btn_speed_t, 2);
	btn_speed_t->setVisible(false);
	
	// 倍速按钮点击事件
	btn_speed->addClickEventListener([pSchedulers, btn_speed, btn_speed_t](Ref* ref) {
		pSchedulers->setTimeScale(2.f);
		btn_speed->setVisible(false);
		btn_speed_t->setVisible(true);
	});
	btn_speed_t->addClickEventListener([pSchedulers, btn_speed, btn_speed_t](Ref* ref) {
		pSchedulers->setTimeScale(1.f);
	...
	// 开始按钮
	Button* btn_g = Button::create("Themes\\Items\\touming-hd\\pause01.png", "Themes\\Items\\touming-hd\\pause02.png");
	btn_g->setPosition(Vec2(825, 605));
	btn_g->setVisible(true);
	this->addChild(btn_g, 2);


	// 菜单按钮
	Button* btn_men = Button::create("Themes\\Items\\touming-hd\\menu01.png", "Themes\\Items\\touming-hd\\menu02.png");
	btn_men->setPosition(Vec2(890, 605));
	this->addChild(btn_men, 2);
	btn_men->addClickEventListener([this, /*btn_speed, */btn_men](Ref* ref)
	{
		Director* director = Director::getInstance();
	...
	// 暂停、开始点击事件
	btn_pause->addClickEventListener([btn_g, btn_pause, btn_stop, btn_speed, btn_men](Ref* ref) {
		btn_g->setVisible(true);
		btn_stop->setVisible(false);
		btn_pause->setVisible(false);
		...
		btn_men->setTouchEnabled(true);
	});
	btn_g->addClickEventListener([btn_g, btn_pause, btn_stop, btn_speed, btn_men](Ref* ref) {
...
void BossScene::create_UpTool(Tower* tower, int time)
{
	Upselect = Sprite::create("Themes\\Items\\Items02-hd\\select_00.png");
	Upselect->setVisible(false);
...
// 创建建塔选择工具
void BossScene::createBuildTool()
{
	// 创建选择工具,暂时隐藏
